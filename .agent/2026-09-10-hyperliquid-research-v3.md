...
- [ ] M1：時刻とデータ品質、共通特徴量、確定証拠の基盤（完了：時点整合の1分足モデル、連続性・OHLCV検証、`common_candles_v1`、研究専用SQLiteの確定episode・review証拠消費、公開足の正規化・JSONL保存、署名なし公開`collect` CLIとsource manifest。残り：部分fillからepisodeへの集約）。
...
- [ ] M4：点評価と3日Replay（完了：確定61本以後の候補化、最大500件、seed固定、非復元、return_5m符号・30分ボラ・出来高z-scoreによる層化順位、選定JSONL/manifest。残り：LLM回答の点評価、3日Replay）。
...
既定のmodel delayは1秒であるため、判断直後の分境界ではなく次の分境界へEntryする。61本の事前足の後に300秒episodeを一つ再生するには、遅延込みで少なくとも68本のfixtureが必要だった。baselineはこの末尾を`incomplete_decisions`として数え、episodeが一件も完結しない入力を`insufficient_data`で返す。

空のJSONLは壊れたファイルではなく「まだ確定足を保存できていない」状態として読込み、baseline CLIは終了コード3と`status=insufficient_data`を返す。見送りのshadowは`simulate_shadow_episode`だけが生成し、episode自身の`kind=shadow`を`VirtualAccount`が見てequityを不変にするため、呼出側が種別を渡し忘れても仮想損益を主口座へ混ぜない。
...
2026-09-10 / Codex：最初のReplay CLIはLLM回答を必要としない3つの固定baselineだけを実装する。確定足の特徴量→決定→Entry/Exit→仮想口座を時刻順に接続し、LLMの揺れ、API費用、Reviewerの影響を混ぜずにSimulatorの会計・時刻境界を検証するため。

2026-09-10 / Codex：M3の次の単位はBatch clientではなく、要求のcanonical identityと送信予約の永続化に限定する。API応答を失った場合に、ローカルのHTTP再試行だけで重複送信を防げるとは扱わず、`submission_unknown`を照会まで停止状態にするため。課金APIを呼ばずに再起動時の安全境界を検証できる。
...
2026-09-12 / Codex：pilot 50と最大500の比較可能性は、要求件数ごとに別抽出せず、一つの層化順位のprefixとして保証する。抽出はreturn_5mの符号、30分実現ボラの全候補中央値、volume z-scoreの符号を使い、同じseedと候補集合なら順序を再現できる。LLM呼び出し・将来損益評価はこの段階に混ぜない。

2026-09-12 / Codex：地点選定のCLIは、評価時点の特徴量だけをJSONLへ保存し、元ローソク足のSHA-256と選定条件を別manifestへ固定する。候補不足を空成果物やゼロ損益へ変換せず、LLM API・Batch・注文APIを一切呼ばない。これにより後続の要求作成は同じ入力資産を再利用できる。

2026-09-12 / Codex：研究の判断頻度は1分足の粒度と分け、UTCの5分slotに固定する。candidateとbaselineが同じslotを使うようにし、選定manifestには候補集合hash、候補/選定後の層別件数、30分実現ボラの分類閾値を保存する。これにより50→500の追加評価で時点や母集団を取り違えない。

2026-09-12 / Codex：Batch API clientより前に、選定済み地点から不変の送信前request artifactを作る。live Gemini gatewayと同じ単一`open_position` schemaを共有し、prompt・strategy・生成設定・featuresをcanonical hashへ含める。準備だけではAPIキーを読まず、課金許可、job作成、SQLite予約、注文を行わないため、入力監査を先に完結できる。
...
2026-09-10 / Codex：見送り方向の評価は通常`simulate_episode`を使い回さず、`simulate_shadow_episode`がtrade decisionへ一時変換して`kind=shadow`を付ける。主口座への反映はepisodeの種別で止め、呼出側の文字列引数だけに安全性を依存させないため。
...
`research/simulator.py`には`simulate_episode(...)`と`simulate_shadow_episode(...)`、`VirtualAccount.apply(...)`を置く。前者は実取引予定の価格・費用関数、shadow版は見送り方向を同じ約定規則で評価して`kind=shadow`を付ける。`VirtualAccount`はshadowの損益を反映せず、通常episodeだけをUTC日次口座へ加算する。`research/baseline.py:run_baseline(...)`は固定3ルールを時刻順に接続する。Replayに実sleepは使わない。
...
以下は実装後に提供するCLI契約。現在は存在しない。作業場所は`C:\Users\Hodaka\Downloads\div\bot_research\hyperliquid-ai-trader`。
    ...
    .\.venv\Scripts\python.exe -m hyperliquid_ai_trader.research.cli baseline --config configs/research/development.json --candles data/research/BTC-1m.jsonl --baseline momentum
...
変更履歴：2026-09-10、M2の固定baseline時系列Replay、CLI、末尾不足の扱い、検証結果を反映した。

変更履歴：2026-09-10、shadow episode、空JSONLの不足データ扱い、simulation設定固定、全111件の検証結果を反映した。

変更履歴：2026-09-10、M3のcanonical request hash、SQLite送信予約、応答不明時の自動再送停止、全113件の検証結果を反映した。
...
変更履歴：2026-09-12、M4の確定1分足候補化、最大500件のseed固定層化順位、pilot prefix性、対象3件の検証結果を反映した。

変更履歴：2026-09-12、M4の`points` CLI、時点安全な特徴量JSONL、元ローソク足hash・層別件数付きmanifest、候補不足時の`insufficient_data`、対象4件の検証結果を反映した。

変更履歴：2026-09-12、研究のUTC 5分判断slot、candidate set hash・候補/選定後の層別件数・ボラティリティ閾値をmanifestへ追加し、baselineと点選定の頻度を一致させた。対象22件の検証結果を反映した。

変更履歴：2026-09-12、M3の送信しない`prepare-requests` CLI、時点安全な選定入力と固定assets/生成設定/schemaを含むcanonical request JSONL、request hash/trial ID/manifest、対象16件の検証結果を反映した。

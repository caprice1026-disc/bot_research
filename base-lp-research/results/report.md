...
- Current progress: `1386` Swap logs, next block `46743327`
...
公式RPCへの負荷を抑えるため、500 block単位、request間隔0.25秒、1回90秒のbounded runとした。初回と再開runで4 chunk（1,386 Swap logs）を取得し、`runs/rpc-swap-7d/results/collection_checkpoint.json` に `next_block=46743327` を保存した。既存の旧範囲データは上書きせず、今回のpilotは別rootへ分離している。
